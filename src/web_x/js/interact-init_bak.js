...
var interval,
	...
	domain = "http://eclassdoc1.qiancloud.com/",
	queue,
	teachingFileList = [],
	empty_teaching_file = {
		"FileID": 1,
...
$(function(){
	if (canvas.getContext){
	  	...
	  	$.ajax({
			url : "/webclass/enterRoom",
			type : "POST",
			dataType : "json",
			data : {"id": $("#webclass-id").val},
			error : function(XMLHttpRequest, errorInfo, e) {
				console.error("Failed to get enterRoom data! error: " + errorInfo);
			},
			success : function(resp) {
				if (resp.success) {
					$(".room-name").html(resp.RoomInfo.RoomName);
					$(".room-time").html(resp.Timestamp+"-"+resp.Timestamp);
					$(".room-id").html(resp.RoomInfo.RoomExID);
					$(".teacher span[class='user-name']").html(resp.UserName);
					// local_user
					local_user.uid = parseInt(resp.UserID);
					local_user.name = resp.UserName;
					local_user.type = parseInt(resp.UserType);
					local_user.token = resp.token;
					// local_group
					local_group.gid = parseInt(resp.RoomInfo.RoomID);
					local_group.title = resp.RoomInfo.RoomName;
					// register
					teachingFileList.push(empty_teaching_file);
					loadImg(empty_teaching_file);
					var etf = '<li cs-fid="'+empty_teaching_file.FileID+'" role="presentation"><a href="#">'+empty_teaching_file.FileName+'</a></li>';
					$(".teaching-file-tab ul").append(etf);
			...
			error: function(xhr, errorMsg, errorObj){
				console.error("load file error! "+errorMsg);
			}
		});
	} else {
...
function itvReg(){
	if (isInGroup) {
		...
		RegisterForce(local_user.uid, local_user.name, local_user.type, "xclass");
...
$("#cvs").on('mousedown', function(e){
	...
	case SHAPE.POINT:
        ...
        $(this).on('mousemove', function(e){
        	...
        	if (coordinate.length > 250) {
        		cdt_move = coordinate[249];
        	}
	...
	case "openfile":
		...
		for (var i = 0; i < teachingFileList.length; i++) {
			if (teachingFileList[i].FileID != fileId) {
				...
				// show teaching file list
				var tfhtml = '<li cs-fid="'+tf.FileID+'" role="presentation"><a href="#">'+tf.FileName+'</a></li>';
...
function toUTF8(str, isGetBytes) {
    var back = [];
    var byteSize = 0;
