...
// JoinGroup 
async function JoinGroup(token, title){
	try{
		if (isEmpty(token)) {
...
function _OnJoinGroup(errorCode, userList){
	if (errorCode === ERROR_CODE.RESULT_SUCCESS) {
		...
		for (let k = 0; k < userList.length; k++) {
			let type = userList[k].type,
				id = userList[k].uid,
				name = userList[k].name;

			if(type === local_user.type){
                showChatMsg('system','系统消息',"你进入了教室!");
            }else{
                showChatMsg('system','系统消息',name+"进入了教室!");
            }
			if (type === 1) {
                pushRoleList('teacher',id,name,0);
			}
            if (type === 2) {
                pushRoleList('student',id,name,0);
            }
		}
		isInGroup = true;
	} else {
...
function _OnLeaveGroup(errorCode){
	if (errorCode === ERROR_CODE.RESULT_SUCCESS) {
		// dosomething
		showChatMsg('system','系统消息',"您已离开教室!");
	} else {
...
function _OnUserLeaveGroup(errorCode, uid, type, name){
	if (errorCode === ERROR_CODE.RESULT_SUCCESS) {
		// dosomething
		showChatMsg('system','系统消息',name+"离开了教室!");
	} else {
...
function _OnServerNotification(errorCode){
	if (errorCode == ERROR_CODE.ERR_UID_REGISTED_RENEW) {
		...
		showChatMsg('system','系统消息',"您已在其他地方登入!");
		reload();
	} else if (errorCode == ERROR_CODE.ERR_CONNECTION_LOST) {
		// lost conn
		showChatMsg('system','系统消息',"您已断开连接!");
